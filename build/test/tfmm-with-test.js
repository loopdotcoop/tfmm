...
(function() {
  var $, $$, Asset, AssetManager, Blot, Flourish, Maestro, Main, MicIn, Tudor, Voice, VoiceSet, tudor, vpSize, ª, ªA, ªB, ªE, ªF, ªI, ªN, ªO, ªR, ªS, ªU, ªV, ªX, ªclone, ªex, ªhas, ªkeymaps, ªpopulate, ªredefine, ªretrieve, ªtype, ªuid,
  ...
  VoiceSet = (function() {
    ...
    VoiceSet.prototype.activate = function() {
      var h, j, len, ref, ref1, results, voice, w;
      ref = vpSize(), w = ref[0], h = ref[1];
      this.size = (Math.min(w, h)) - 20;
      ª(w, h, this.size);
      this.size = 512;
      this.$canvas.setAttribute('width', this.size + 'px');
      this.$canvas.setAttribute('height', this.size + 'px');
      this.$voiceSet.setAttribute('width', this.size + 'px');
      this.$voiceSet.setAttribute('height', this.size + 'px');
      ref1 = this.voices;
      results = [];
