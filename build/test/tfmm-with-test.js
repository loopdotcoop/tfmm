...
(function() {
  var Tfmm, Tudor, Voice, tudor, ª, ªA, ªB, ªE, ªF, ªI, ªN, ªO, ªR, ªS, ªU, ªV, ªX, ªclone, ªex, ªhas, ªpopulate, ªredefine, ªretrieve, ªtype, ªuid,
  ...
  Tfmm = (function() {
    ...
    function Tfmm(config) {
      ...
      if (5 < this.COLORS.length) {
        throw new Error("'" + config.$player.id + "' frontmatter contains " + this.COLORS.length + " colors");
      }
      if (config.$player) {
        this._canvas = document.createElement('canvas');
        this._canvas.setAttribute('width', '256px');
        this._canvas.setAttribute('height', '256px');
        this._canvas.setAttribute('class', 'main');
        this._ctx = this._canvas.getContext('2d');
        config.$player.appendChild(this._canvas);
        this._voices = [];
        ref = this.COLORS;
    ...
    Tfmm.prototype.render = function(secfrac) {
      var j, len, ref, results, voice;
      this._ctx.clearRect(0, 0, 256, 256);
      this._ctx.fillStyle = "rgb(0,100,0)";
      this._ctx.fillRect(0, 0, secfrac, secfrac);
      ref = this._voices;
      results = [];
      for (j = 0, len = ref.length; j < len; j++) {
        voice = ref[j];
        results.push(voice.render(secfrac));
      }
      return results;
    };
  ...
  Voice = (function() {
    ...
    function Voice(config) {
      ...
      this._bCtx = $_button.getContext('2d');
      config.$player.appendChild($_button);
      this._bCtx.fillStyle = config.color;
    }

    Voice.prototype.render = function(secfrac) {
      this._bCtx.clearRect(0, 0, 64, 64);
      this._bCtx.fillRect(0, 0, secfrac, secfrac);
      this._mCtx.fillStyle = this.color;
      return this._mCtx.fillRect(50, 50, secfrac, secfrac);
    };
